...
class Telegram {
    ...
    sendGifFromUrl(chatId, url, options, callback) {
        callback = callback || Function()
        ...
        wstream.on('finish', () => {
            self.sendVideo(chatId, fs.createReadStream(__dirname + '/temp/' + fileName), options, (body, err) => {
                fs.unlink(__dirname + '/temp/' + fileName)
                if (!err) {
                    callback(body)
                } else {
    ...
    _createScope(chatId, user, update) {
        ...
        var funcs = [
            ...
            'sendMessage', 'sendLocation', 'sendAudio', 'sendAudioFromUrl', 'forwardMessage', 'sendGifFromUrl',
